...
$root = $PSScriptRoot
$project = Join-Path $root 'DadsDevCommands.csproj'
$dll = Join-Path $root 'bin\Release\net48\DadsDevCommands.dll'
$packageRoot = Join-Path $root 'package'
$manifestPath = Join-Path $packageRoot 'manifest.json'

dotnet build $project -c Release
if ($LASTEXITCODE -ne 0) { throw "DadsDevCommands build exited with code $LASTEXITCODE" }
...
if ($Package) {
    ...
    $folder = Join-Path $dist "DadsDevCommands-$($manifest.version_number)"
    $zipPath = Join-Path $dist "DadsDevCommands-$($manifest.version_number).zip"
    ...
    try {
        ...
        foreach ($forbidden in @('BepInEx.dll', '0Harmony.dll', 'assembly_valheim.dll', 'YamlDotNet.dll')) {
            if ($forbidden -in $names) { throw "ZIP contains forbidden dependency assembly: $forbidden" }
        }
    }
    finally { $zip.Dispose() }
...
if ($Install) {
    ...
    $installPath = Join-Path $pluginRoot 'Dad_Is_Bored-DadsDevCommands'
    $archiveRoot = Join-Path $root 'Archive\installed-builds'
    New-Item -ItemType Directory -Path $archiveRoot -Force | Out-Null
    if (Test-Path -LiteralPath $installPath -PathType Container) {
        $stamp = Get-Date -Format 'yyyyMMdd-HHmmss-fff'
        Move-Item -LiteralPath $installPath -Destination (Join-Path $archiveRoot "Dad_Is_Bored-DadsDevCommands-$stamp")
    }
    New-Item -ItemType Directory -Path $installPath | Out-Null
    if ($Package) {
        $builtPackage = Join-Path $root "dist\DadsDevCommands-$($manifest.version_number)"
        Get-ChildItem -LiteralPath $builtPackage -File | Copy-Item -Destination $installPath
    }
    else {
        Copy-Item -LiteralPath $dll -Destination (Join-Path $installPath 'DadsDevCommands.dll')
    }
